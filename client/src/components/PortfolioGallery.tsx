...
export default function PortfolioGallery({ service }: GalleryProps) {
  ...
  // Limita a exibição para até 10 imagens.
  const images = service.gallery.slice(0, 10);
  
  // O movimento agora calcula a largura inteira do scroller para rolar de 3 em 3 no PC ou 1 em 1 no mobile
  ...
  return (
    <div className="portfolio-gallery">
      <div className="mb-5 flex items-end justify-between gap-6">
        <div>
            <p className="eyebrow text-ocre">{service.eyebrow}</p>
            <h3 className="mt-2 font-display text-3xl text-forest md:text-4xl">{service.title}</h3>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink/60">{service.portfolioDescription}</p>
        </div>
        {/* Ajustado: Removido o 'hidden md:flex' para que as setas apareçam no mobile também */}
        <div className="flex items-center gap-2">
            <button type="button" onClick={() => move(-1)} className="grid size-10 place-items-center rounded-full border border-ink/15 text-forest hover:border-ocre hover:text-ocre" aria-label={`Voltar nas imagens de ${service.title}`}><ArrowLeft size={16} /></button>
            <button type="button" onClick={() => move(1)} className="grid size-10 place-items-center rounded-full border border-ink/15 text-forest hover:border-ocre hover:text-ocre" aria-label={`Avançar nas imagens de ${service.title}`}><ArrowRight size={16} /></button>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-ink/40">arraste horizontalmente para explorar</span>
          <Link href={`/servicos/${service.slug}`} className="inline-flex items-center gap-2 text-sm font-bold text-forest underline decoration-ocre decoration-2 underline-offset-8">Ver mais <ArrowRight size={15} /></Link>
      </div>
